...
function nhlBoard(data) {
  ...
  for (const g of data.games || []) {
    ...
    byDay.get(key).games.push({
      ...
      cells, pick: pickView(p, unit, { noneLabel: pre ? "Exhibition · model abstains" : undefined }),
  ...
  const strip = [
    ...
    fw && fw.sealed ? { label: "Forward ledger · sealed", value: fw.rows ? String(fw.rows) : dash, sub: fw.rows ? `settled rows across ${fw.markets} markets` : "Return is decided at season's end, not before", fine: "" }
      : fw ? { label: "Forward ledger · ROI", value: `${F.fmtSigned(fw.roiPct)}%`, sub: `95% ${F.fmtSigned(fw.ciLow)}% to ${F.fmtSigned(fw.ciHigh)}% · n=${fw.wagers}`, fine: `CLV ${F.fmtSigned(fw.clvPct)}%` }
      : { label: "Forward ledger", value: dash, sub: "No settled opinions yet", fine: "" },
  ];
